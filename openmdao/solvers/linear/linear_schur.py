...
class LinearSchur(BlockLinearSolver):
    ...
    **kwargs : dict
    ...
    def solve(self, mode, rel_systems=None):
        ...
        subsys2_outputs = subsys2._vectors["output"]["linear"]
        subsys2_residuals = subsys2._vectors["residual"]["linear"]
        ...
        if mode == "fwd":
            ...
            b_vec = subsys1._vectors["residual"]["linear"]
            # off = b_vec._root_offset
            b_vec2 = subsys2._vectors["residual"]["linear"]
            # off2 = b_vec2._root_offset
            off = 0
            off2 = len(b_vec)
            ...
            scope_out, scope_in = system._get_scope(subsys2)
            # scope_out = self._vars_union(self._scope_out, scope_out)
            # scope_in = self._vars_union(self._scope_in, scope_in)
            ...
            # for ii, var in enumerate(vars_to_solve):
            subsys2._vectors["output"]["linear"].set_val(d_subsys2)
            ...
            subsys1._solve_linear(mode, self._rel_systems)
            ...
            # system._transfer("linear", mode, subsys2.name)
            b_vec = subsys1._vectors["output"]["linear"]
            # b_vec_cache1 = subsys1._doutputs.asarray(copy=True)
            # off = b_vec._root_offset

            b_vec2 = subsys2._vectors["output"]["linear"]
            ...
            # loop over the variables just to be safe with the ordering
            # for ii, var in enumerate(resd_to_solve):
            subsys2._vectors["residual"]["linear"].set_val(d_subsys2)
